...
import { Search, Favorite, FavoriteBorder, Edit, Settings } from '@mui/icons-material';
...
export const DashboardPage: React.FC = () => {
    ...
    // Recent devices (laatst gewijzigd)
    ...
    const handleDeviceControl = (device: Device, newValue: DeviceValue) => {
        // Hier zou je de device update logica implementeren
        console.log('Updating device:', device.id, newValue);
        // updateDeviceMutation.mutate({ id: device.id, data: { waarde: newValue } });
    };
                ...
                <CardContent>
                    <Box sx={{
                        ...
                        <Box sx={{ minWidth: 120 }}>
                            <FormControl fullWidth size="small">
                                ...
                                >
                                    ...
                                    {rooms
                                        .filter(room => selectedFloor === 'all' || room.verdiepingId === selectedFloor)
                                        .map(room => (
                        ...
                        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                            <Chip
                                label={`${filteredDevices.length} devices`}
                                color="primary"
                                variant="outlined"
                            />
                            {favorites.size > 0 && (
                                <Chip
                                    label={`${favorites.size} favorieten`}
                                    color="secondary"
                                    variant="outlined"
                                />
                            )}
